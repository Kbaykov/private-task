...
(
	ID int Identity(1,1) Primary key,
	Elevator_ID int,
	Request_floor tinyint, 
	Requested_floor tinyint,
...
begin try 
	...
	begin 
	 --optimization of which elevator will be assigned with the request 
		...
		select top 1 @p_elevator_id, @p_request_floor_id, Requested_floor, @p_direction, isnull(Transported_Load, 0), @p_is_external
		from dbo.Active_Queue a
...
go
/*****************************************************/
 create or alter procedure SP_Maintanance_Report
   @p_elevator_id int = 0,
   @p_request_id int = 0
as 
begin try 
	...
	if @p_request_id = 0 --check for maintananec if not called for certain request
		...
		set is_for_maintenance = case when @p_max_floors <= current_traveled_floors then 'For Maintenance'
									  ...
									  else 'Elevator haven`t reached maintenance limit'
									  end
